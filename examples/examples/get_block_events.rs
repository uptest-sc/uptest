...
use libuptest::ws_mod::{get_raw_metadata, blocknumber_to_blockhash, get_block_events};
...
async fn main() -> anyhow::Result<()> {
    println!("Getting the latest pallet functions that where executed in the latest finalized block");
    ...
    let mut subscrib: SubscriptionWrapper<Header> = client.clone().subscribe::<Header>("chain_subscribeFinalizedHeads", RpcParams::new(), "chain_unsubscribeFinalizedHeads").unwrap();
  ...
  //  for _ in 0..3 {
    ...
    let blocknr = nextone.unwrap().unwrap().number;
      ...
      let blockhash: H256 = blocknumber_to_blockhash(client.clone(), blocknr.clone()).await.unwrap();
...
//    println!("Got preblock: {:?}", preblock.block);
